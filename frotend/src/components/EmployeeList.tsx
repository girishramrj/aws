...
const EmployeeList = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      // Initialize empty addresses array if not present
      const employeesWithAddresses = data.map((emp: any) => ({
        ...emp,
        addresses: emp.addresses || []
      }));
      
      setEmployees(employeesWithAddresses);
      setFilteredEmployees(employeesWithAddresses);
  ...
  const handleDelete = async () => {
    try {
      ...
      setEmployees(employees.filter(emp => !confirmDelete.ids.includes(emp.id || 0)));
      setFilteredEmployees(filteredEmployees.filter(emp => !confirmDelete.ids.includes(emp.id || 0)));
      setSelected(selected.filter(id => !confirmDelete.ids.includes(id)));
      setConfirmDelete({ show: false, ids: [] });
      
      // Show success message
      alert(confirmDelete.ids.length === 1 
        ? 'Employee deleted successfully' 
        : `${confirmDelete.ids.length} employees deleted successfully`);
    } catch (error) {
      console.error('Error deleting employee(s):', error);
      alert('Error deleting employee(s). Please try again.');
    }
  };
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      // Select all filtered employees, not all employees
  ...
  const handleViewEmployee = () => {
    if (selectedEmployee && selectedEmployee.id) {
      // Fetch the employee details with addresses
      ...
      >
        <MenuItem onClick={handleViewEmployee}>
          <ListItemIcon>
            <VisibilityIcon 
              fontSize="small" 
              sx={{ color: isDarkMode ? '#3DC2EC' : 'info.main' }} 
            />
          </ListItemIcon>
          <ListItemText sx={{ color: isDarkMode ? 'white' : 'inherit' }}>View Details</ListItemText>
        </MenuItem>
        <MenuItem onClick={handleEditEmployee}>
          <ListItemIcon>
            <EditIcon 
              fontSize="small" 
              sx={{ color: isDarkMode ? '#4C3BCF' : 'primary.main' }} 
            />
          </ListItemIcon>
          <ListItemText sx={{ color: isDarkMode ? 'white' : 'inherit' }}>Edit</ListItemText>
        </MenuItem>
        <MenuItem onClick={handleDeleteEmployee}>
          <ListItemIcon>
            <DeleteIcon 
              fontSize="small" 
              sx={{ color: isDarkMode ? '#f44336' : 'error.main' }} 
            />
          </ListItemIcon>
          <ListItemText sx={{ color: isDarkMode ? 'white' : 'inherit' }}>Delete</ListItemText>
        </MenuItem>
      </Menu>
